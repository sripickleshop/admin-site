...
const CustomersModule = {
    ...
    async fetchCustomerData() {
        try {
            ...
            document.getElementById('customers-table-body').innerHTML = `<tr><td colspan="5" class="px-6 py-8 text-center text-red-500">Failed to load customer insights.</td></tr>`;
        }
    },
    ...
    renderCustomers() {
        ...
        if (viewContainer.querySelector('h2')) {
                ...
                <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                        ...
                        <div class="flex gap-2">
                             ...
                             <button class="bg-white border border-gray-300 text-gray-600 px-3 py-1.5 rounded text-sm font-bold hover:bg-gray-50 flex items-center gap-2">
    ...
    renderRows(list) {
        ...
        tbody.innerHTML = list.map(cust => {
            ...
            const avatarHtml = cust.avatar_url
                ? `<img src="${cust.avatar_url}" class="w-10 h-10 rounded-full object-cover border border-gray-200" alt="${cust.name}">`
            ...
            <tr class="hover:bg-gray-50 transition-colors cursor-pointer" onclick="CustomersModule.openCustomerModal('${cust.id}')">
                <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                        ${avatarHtml}
                        <div>
                            <p class="font-bold text-gray-800">${cust.name} ${cust.type === 'Guest' ? '<span class="text-xs font-normal text-gray-400 border border-gray-200 rounded px-1 ml-1">Guest</span>' : ''}</p>
                            <p class="text-xs text-gray-500 flex items-center gap-1">
                                ${cust.email !== '-' ? cust.email : ''}
                                ${cust.phone ? `<span><i data-lucide="phone" class="w-3 h-3 inline"></i> ${cust.phone}</span>` : ''}
                                ${(cust.email === '-' && !cust.phone) ? 'No Contact Info' : ''}
                            </p>
                        </div>
                    </div>
                </td>
                <td class="px-6 py-4 text-gray-600">${cust.city}</td>
